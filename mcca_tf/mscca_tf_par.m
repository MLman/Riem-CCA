function [wx, wy, r, u, v, precomp] = mscca_tf_par(X,Y, varargin)
...
%     Yvecs = precomp.Yvecs;
    ...
    muX = karcher_mean_spd_tf_par(X);
    muY = karcher_mean_spd_tf_par(Y);

    % Logarithm map
    disp('Logmap Calculation');
    Xwrs = logmap_pt2array_spd_tf_par(muX,X);
    Ywrs = logmap_pt2array_spd_tf_par(muY,Y);

    % Embedding product space of Euclidean space (R6)
    disp('Embedding');
    Xvecs = embeddingR6_vecs_tf_par(muX,Xwrs);
    Yvecs = embeddingR6_vecs_tf_par(muY,Ywrs);
...
wx = invembeddingR6_vecs_tf_par(muX, wxvec);
wy = invembeddingR6_vecs_tf_par(muY, wyvec);
